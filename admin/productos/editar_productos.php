      ...
      if(!file_exists('../')){
          ...
          if(file_exists('../../')){
               if(move_uploaded_file($guardado2, '../../images/' .$nombreIMG2)){
                   $sql4 ="UPDATE `productos` SET `imagen01` = '$nombreIMG2' WHERE `productos`.`id` = '$id'";
    }
             }else{
                 $sql4 ="UPDATE `productos` SET `imagen01` = '$nombreIMG2' WHERE `productos`.`id` = '$id'";
             }
      ...
      if(!file_exists('../')){
          ...
          if(file_exists('../../')){
               if(move_uploaded_file($guardado3, '../../images/' .$nombreIMG3)){
                    $sql3 ="UPDATE `productos` SET `imagen02` = '$nombreIMG3' WHERE `productos`.`id` = '$id'";
               }
             }else{
                $sql3 ="UPDATE `productos` SET `imagen02` = '$nombreIMG3' WHERE `productos`.`id` = '$id'";
             }
...
//-------------------------------------------//
$sql = "UPDATE productos SET nombre = '$nombre' , descripcion = '$descripcion' , cantidad = '$cantidad' , categoria= '$categoria', precio = '$precio' WHERE productos .id = '$id'";
