<?php 
   ...
   if(!empty($nombre && $apellido && $direccion && $direccion && $email && $hash && $celular && $ciudad && $codPostal && strlen($password)>=8 && strlen($nombre)>3 && strlen($apellido) > 3 && strlen($direccion)>3 && strlen($ciudad)>3 && strlen($codPostal)>3 && strlen($codPostal)< 5 && preg_match('/^(?=.{3,36}$)[a-zñA-ZÑ](\s?[a-zñA-ZÑ])*$/',$nombre) && preg_match('/^(?=.{3,36}$)[a-zñA-ZÑ](\s?[a-zñA-ZÑ])*$/',$apellido) && preg_match('/^(?=.{3,36}$)[a-zñA-ZÑ](\s?[a-zñA-ZÑ])*$/',$ciudad) && preg_match('/^[A-z0-9\\._-]+@[A-z0-9][A-z0-9-]*(\\.[A-z0-9_-]+)*\\.([A-z]{2,6})$/',$email) && preg_match('/^(?:(?:00)?549?)?0?(?:11|[2368]\d)(?:(?=\d{0,2}15)\d{2})??\d{8}$/D',$celular) && preg_match('/^[0-9]+$/' , $codPostal) && !preg_match('/[$%&|<>#]/', $direccion)))
    {
